// nvcc -lineinfo -res-usage -arch=sm_75 -std=c++11 22111090-prob1-v3.cu -o 22111090-prob1-v3
...
int main() {
  ...
  cudaCheckError(cudaMallocManaged((void**)&h_input, SIZE * sizeof(float)));
  cudaCheckError(cudaMallocManaged((void**)&h_serial, SIZE * sizeof(float)));
  cudaCheckError(cudaMallocManaged((void**)&h_kernel, SIZE * sizeof(float)));
  ...
  /*  Unified Virtual Memory Kernel implementation starts */
  ...
  for (int block_size : block_sizes) {
    ...
    kernel<<<gridSize, blockSize, sz * sizeof(float)>>>(h_input, h_kernel);
    cudaCheckError(cudaPeekAtLastError());
    cudaCheckError(cudaDeviceSynchronize());
    cudaEventRecord(end);
    cudaEventSynchronize(end);
    cudaEventElapsedTime(&kernel_time, start, end);
    std::cout << "UVM Kernel block size: "<< block_size << ", time (ms): " << kernel_time << "\n";
