// nvcc -lineinfo -res-usage -arch=sm_75 -std=c++14 22111090-prob3-v1.cu -o 22111090-prob3-v1
...
void gridloopsearch(double *a, double *b, double kk) {
    ...
    gridloopsearch_kernel<<<gridSize, blockSize>>>(d_pnts, d_results, d_a, d_b, d_kk);
    ...
    cudaCheckError(cudaMemcpy(&h_pnts, d_pnts, sizeof(int), cudaMemcpyDeviceToHost));
    cudaCheckError(cudaMemcpy(h_results, d_results, max_results * 10 * sizeof(double), cudaMemcpyDeviceToHost));
