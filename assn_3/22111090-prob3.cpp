// Compile: g++ -msse4 -mavx2 -mavx512f -march=native -O3 -fopt-info-vec-optimized -fopt-info-vec-missed -o prob3 22111090-prob3.cpp
...
int avx2_version(int *__restrict__ source, int *__restrict__ dest)
{
  __builtin_assume_aligned(source, ALIGN);
  __builtin_assume_aligned(dest, ALIGN);

  const int avx2_stride = AVX2_WIDTH_BITS / (sizeof(int) * 8);
  const int sse4_stride = SSE_WIDTH_BITS / (sizeof(int) * 8);

  for(int i = 0; i < N; i += avx2_stride) {
    __m256i Y =_mm256_load_si256((__m256i *) &source[i]);
    Y = _mm256_add_epi32(Y, _mm256_slli_si256(Y, 4));
    Y = _mm256_add_epi32(Y, _mm256_slli_si256(Y, 8));
    _mm256_store_si256( (__m256i *) &dest[i], Y);
  }

  int x = 0;
  __m128i prefix=(__m128i)_mm_broadcast_ss((float *) &x);

  for(int i=0;i < N;i += sse4_stride) {

    __m128i Y=_mm_load_si128((__m128i *) &dest[i]);

    x += dest[i+3];

    Y = _mm_add_epi32(Y, prefix);
    prefix=(__m128i)_mm_broadcast_ss((float *) &x);
    _mm_store_si128( (__m128i *) &dest[i], Y);
  }

  return dest[N-1];
}

int avx512_version(int *__restrict__ source, int *__restrict__ dest)
{
  __builtin_assume_aligned(source, ALIGN);
  __builtin_assume_aligned(dest, ALIGN);

  const int avx512_stride = AVX512_WIDTH_BITS / (sizeof(int) * 8);

  for (int i = 0; i < N; i += avx512_stride)
  {
      __m512i Y = _mm512_load_si512((__m512i *)&source[i]);
      Y = _mm512_add_epi32(Y, _mm512_alignr_epi32(Y, Y, 1));
      Y = _mm512_add_epi32(Y, _mm512_alignr_epi32(Y, Y, 2));
      Y = _mm512_add_epi32(Y, _mm512_alignr_epi32(Y, Y, 4));
      Y = _mm512_add_epi32(Y, _mm512_alignr_epi32(Y, Y, 8));
      _mm512_store_si512((__m512i *)&dest[i], Y);
  }

  int x = 0;
  __m512i prefix = _mm512_set1_epi32(x);

  for (int i = 0; i < N; i += avx512_stride)
  {
      __m512i Y = _mm512_load_si512((__m512i *)&dest[i]);

      x += dest[i + 7];

      Y = _mm512_add_epi32(Y, prefix);
      prefix = _mm512_set1_epi32(x);
      _mm512_store_si512((__m512i *)&dest[i], Y);
  }
